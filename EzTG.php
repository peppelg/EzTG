...
class EzTG {
  ...
  class EzTGKeyboard {
    public function __construct($type='keyboard', $rkm=array('resize_keyboard' => true, 'keyboard' => array()), $secure=false) {
      ...
      $this->secure = $secure;
      if ($type === 'inline')
      $this->keyboard = array('inline_keyboard' => array());
      else
      $this->keyboard = $rkm;
      return $this;
    }
    public function add($text, $callback_data=NULL, $type='auto') {
      if ($this->type === 'inline') {
        ...
        if ($type === 'callback_data' and $this->secure) {
          $callback_data = '!'.json_encode(array('c' => $callback_data, 'h' => hash('crc32b', $callback_data.';'.$this->secure)));
        }
        array_push($this->keyboard['inline_keyboard'][$this->line], array('text' => $text, $type => $callback_data));
      } else {
